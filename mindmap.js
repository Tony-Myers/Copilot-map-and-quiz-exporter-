function normaliseMindMapNodes() {
  if (!studyPayload.mind_map) {
    studyPayload.mind_map = { title: "Mind map", layout: "radial", nodes: [] };
  }

  if (!Array.isArray(studyPayload.mind_map.nodes)) {
    studyPayload.mind_map.nodes = [];
  }

  const nodes = studyPayload.mind_map.nodes;
  const byId = new Map();

  // First pass: ensure required fields exist
  nodes.forEach((node, index) => {
    if (!node.id) node.id = index === 0 ? "root" : `node_${index}`;
    if (node.parent === undefined) node.parent = null;
    if (!node.label) node.label = "Untitled node";
    if (!node.source_ref) node.source_ref = "";
    if (!node.note) node.note = "";
    if (!node.url) node.url = "";
    if (!node.attachedImage) node.attachedImage = "";
    if (!Array.isArray(node.attachedLinks)) node.attachedLinks = [];
    if (!Array.isArray(node.attachedDocs)) node.attachedDocs = [];
  ...
  if (!byId.has("root")) {
    const rootNode = {
      id: "root",
      label: studyPayload.mind_map.title || studyPayload.source?.title || "Main topic",
      parent: null,
      source_ref: "",
      note: "",
      url: "",
  ...
  // Include anything missed
  nodes.forEach(node => {
    if (!visited.has(node.id)) ordered.push(node);
  });

  studyPayload.mind_map.nodes = ordered;
}

function renderMindMap() {
  normaliseMindMapNodes();
  ...
  studyPayload.mind_map.nodes.forEach((node, index) => {
    ...
    const parentOptions = studyPayload.mind_map.nodes
      ...
      .map(
        n =>
          `<option value="${escapeAttr(n.id)}" ${node.parent === n.id ? "selected" : ""}>${escapeHtml(n.label)}</option>`
      )
      .join("");
    ...
    card.innerHTML = `
      ...
      <div class="node-fields">
        ...
        <div>
          ...
          <textarea rows="2" data-field="note" data-node-id="${escapeAttr(node.id)}">${escapeHtml(node.note || "")}</textarea>
        </div>
        <div>
          <label class="label">Primary URL</label>
          <input type="text" data-field="url" data-node-id="${escapeAttr(node.id)}" value="${escapeAttr(node.url || "")}" placeholder="https://...">
        </div>
        <div>
          <label class="label">Attached image URL / data URI</label>
          <input type="text" data-field="attachedImage" data-node-id="${escapeAttr(node.id)}" value="${escapeAttr(node.attachedImage || "")}" placeholder="Optional">
        </div>
        <div>
          ...
          ${
            node.id === "root"
              ...
              : `<select data-field="parent" data-node-id="${escapeAttr(node.id)}"><option value="">(none)</option>${parentOptions}</select>`
          }
        </div>
...
function bindMindMapEvents() {
  ...
  document.querySelectorAll('[data-field="url"]').forEach(input =>
    input.addEventListener("input", e => {
      const node = findNodeById(e.target.dataset.nodeId);
      if (node) {
  ...
  document.querySelectorAll('[data-field="attachedImage"]').forEach(input =>
    input.addEventListener("input", e => {
      const node = findNodeById(e.target.dataset.nodeId);
      if (node) {
        node.attachedImage = e.target.value;
        updatePayloadPreview();
        renderMindMap();
      }
    })
  ...
  document.querySelectorAll('[data-field="attachedLinks"]').forEach(area =>
    area.addEventListener("input", e =>
      parseJsonFieldToNode(e.target.dataset.nodeId, "attachedLinks", e.target.value)
    )
  );

  document.querySelectorAll('[data-field="attachedDocs"]').forEach(area =>
    area.addEventListener("input", e =>
      parseJsonFieldToNode(e.target.dataset.nodeId, "attachedDocs", e.target.value)
    )
  );
...
function parseJsonFieldToNode(nodeId, field, text) {
  ...
  try {
    ...
    // ignore partial invalid typing while user edits
  }
}

function createDefaultNode(counter, parentId) {
  return {
    id: `node_${counter}`,
    label: "New node",
...
function addChildNodeTo(parentId) {
  studyPayload.mind_map.nodes.push(createDefaultNode(generateNodeNumericId(), parentId));
}
...
function generateNodeNumericId() {
  let counter = 1;
  while (studyPayload.mind_map.nodes.some(n => n.id === `node_${counter}`)) counter += 1;
  return counter;
}
...
function buildMindMapTree() {
  ...
  studyPayload.mind_map.nodes
    ...
    .forEach(node => rootUl.appendChild(buildMindMapTreeNode(node)));
